...
class WildcatPool2dFunction(Function):
    ...
    def forward(self, input):
        ...
        if kmax > 0 and self.alpha is not 0:
    ...
    def backward(self, grad_output):
        ...
        grad_input = grad_output.new().resize_(batch_size, num_channels, n).fill_(0)\
            .scatter_(2, self.indices_max, grad_output_max).div_(kmax)
        ...
        if kmin > 0 and self.alpha is not 0:
            ...
            grad_input_min = grad_output.new().resize_(batch_size, num_channels, n).fill_(0)\
                .scatter_(2, self.indices_min, grad_output_min).mul_(self.alpha / kmin)
...
class WildcatPool2d(nn.Module):
    ...
    def __repr__(self):
        return self.__class__.__name__ + \
               ' (kmax=' + str(self.kmax) + ', kmin=' + str(self.kmin) + ', alpha=' + str(self.alpha) + ')'



'''
WILDCAT Class Pooling
'''
class ClassWisePoolFunction(Function):
    ...
    def forward(self, input):
        ...
        if num_channels % self.num_maps != 0:
            print('Error in ClassWisePoolFunction. '
                  'The number of channels has to be a multiple of the number of maps per class')
            sys.exit(-1)
